...
# NLP models (ruh roh!)
def test_onnx_bert():
    model_path = "/Users/andrewzhaoluo/Downloads/bertsquad-8.onnx"
    onnx_model = onnx.load(model_path)
    mod, mod_params = relay.frontend.from_onnx(
        onnx_model,
        shape={
            "unique_ids_raw_output___9:0": [100],
            "segment_ids:0": [100, 256],
            "input_mask:0": [100, 256],
            "input_ids:0": [100, 256],
        },
    )
    mod_params["unique_ids_raw_output___9:0"] = np.random.randint(0, 100, size=[100])
    mod_params["segment_ids:0"] = np.random.randint(0, 100, size=[100, 256])
    mod_params["input_mask:0"] = np.random.randint(0, 100, size=[100, 256])
    mod_params["input_ids:0"] = np.random.randint(0, 100, size=[100, 256])
    output_mod = verify_fp32_fp16_output_close(mod, mod_params, atol=0.05, rtol=0.01)
    assert not tvm.ir.structural_equal(mod, output_mod)
...
if __name__ == "__main__":
    ...
    config = transformers.DistilBertConfig(
        vocab_size=30, max_position_embeddings=128, n_layers=2, n_heads=4, dim=100, hidden_dim=400
    )
    model = transformers.DistilBertModel(config)
    input = torch.randint(0, 10, size=[10, 100], dtype=torch.long)
    torch.onnx.export(model, input, "/Users/andrewzhaoluo/Downloads/distilbert.onnx")
    """
