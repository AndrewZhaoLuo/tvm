...
namespace relay {
  ...
  class DivToMulRewrite: public MixedModeMutator {
    Expr Rewrite_(const CallNode* pre, const Expr& post) override final {
    if (const CallNode* call_node = post.as<CallNode>()) {
      if (call_node->op == Op::Get("divide")) {
          auto rhs = call_node->args[1].as<ConstantNode>();
          if(rhs != nullptr) {
            auto inv = runtime::NDArray::Empty(rhs->data.Shape(), rhs->data.DataType(), rhs->data->device);
...
TVM_REGISTER_GLOBAL("relay._transform.DivToMul")
    .set_body_typed(DivToMul);
