...
class FP16GraphCreator : public ExprMutator {
 private:
  ...
  ColorFunc colorer;
  OutputDtypeFunc output_dtype_func;

  Attrs GetNewAttrs(const CallNode* call, DataType accumulation_dtype) {
  ...
  template <typename T>
  void ModifyAttrsOutputDType(const T* attrs, DataType accumulation_dtype) {
  ...
  template <typename T>
  void ModifyAttrsDType(const T* attrs, DataType accumulation_dtype) {
  ...
  Expr CachedCast(Expr expr, DataType expr_dtype, DataType wanted_dtype) {
  ...
  Expr CastArg(Expr expr, Type expr_type, DataType wanted_dtype) {
  ...
  std::pair<Array<Expr>, Array<Type>> CastAllArgs(Array<Expr>& cur_args, Array<Type>& cur_arg_types,
...
Expr RewriteFp16Graph(const Expr& expr, ColorFunc colorer, OutputDtypeFunc output_dtype_func) {
